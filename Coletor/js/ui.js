// ===================================================================================
//  MÓDULO DE GERENCIAMENTO DA INTERFACE DO USUÁRIO (UI) - VERSÃO CORRIGIDA
// ===================================================================================
...
const UI = {
    ...
    async displayHistory(talhaoName, fazendaName, talhaoId) {
        ...
        try {
            const [syncedHistory, localAnotacoes] = await Promise.all([ getHistory(), getAnotacoes(talhaoId) ]);
            ...
            const historyHtml = allRecords.map(record => {
                if (record.type === 'monitoring') return this.createMonitoringCard(record, fazendaName, talhaoName);
                if (record.type === 'annotation') return this.createAnnotationCard(record, fazendaName, talhaoName);
                return '';
            }).join('');
    ...
    createAnnotationCard(record, fazendaName, talhaoName) {
        ...
        const sharePayload = { fazenda: fazendaName, talhao: talhaoName, date: annotationDate, text: textContent, imageUrl: imageUrl };
        // CORREÇÃO APLICADA: Usa encodeURIComponent para garantir que o JSON seja seguro para o HTML
        const payloadString = encodeURIComponent(JSON.stringify(sharePayload));
        
        const imageHtml = imageUrl ? `<img src="${imageUrl}" alt="Foto da anotação" class="annotation-photo" loading="lazy" onclick="UI.showFullscreenImage('${imageUrl}')">` : '';
        const shareButtonHtml = `<button class="share-btn" onclick="UI.shareAnnotationData('${payloadString}')"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M15 8a3 3 0 10-2.977-2.63l-4.94 2.47a3 3 0 100 4.319l4.94 2.47a3 3 0 10.895-1.789l-4.94-2.47a3.027 3.027 0 000-.74l4.94-2.47C13.456 7.68 14.19 8 15 8z" /></svg></button>`;
    ...
    async handleDeleteRecord(recordId, talhaoId, recordType, buttonElement) {
        ...
        if (confirm(confirmationMessage)) {
            ...
            if (isSynced) await DataManager.deleteMonitoringRecord(recordId);
            else if (recordType === 'annotation' && talhaoId) await deleteAnotacao(talhaoId, recordId);
