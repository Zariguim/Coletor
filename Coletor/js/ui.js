...
const UI = {
    ...
    async displayHistory(talhaoName, fazendaName, talhaoId) {
        ...
        try {
            ...
            const typedRecords = allHistory.map(r => ({ ...r, type: r.tag === 'Anotação' ? 'annotation' : (r.tag === 'Demarcação' ? 'demarcation' : (r.tag === 'População' ? 'population' : 'monitoring')) }));
    ...
    createPopulationCard(record) {
        ...
        const espacamentoIndex = header.indexOf('espacamento_m');
        const amostrasIndex = header.indexOf('numero_amostras');
        const mediaIndex = header.indexOf('plantas_por_ha');
        const metrosAvaliadosIndex = header.indexOf('metros_avaliados_amostra');

        const espacamento = espacamentoIndex !== -1 ? data[espacamentoIndex] : 'N/D';
        const numAmostras = amostrasIndex !== -1 ? data[amostrasIndex] : 'N/D';
        const mediaPlantas = mediaIndex !== -1 ? parseInt(data[mediaIndex]) : 0;
        const metrosAvaliados = metrosAvaliadosIndex !== -1 ? data[metrosAvaliadosIndex] : 'N/D';
        
        const sharePayload = { 
            fazenda: record.fazenda, 
            talhao: record.talhao, 
            date: recordDate, 
            media: mediaPlantas.toLocaleString('pt-BR'),
            espacamento: espacamento,
            amostras: numAmostras,
            metros: metrosAvaliados
        };
        ...
        if (confirm(confirmationMessage)) {
            ...
            if (recordType === 'annotation') {
                if (isLocal) {
                    ...
                    // Se for uma anotação já sincronizada (ou seja, tem um ID numérico do Supabase),
                    // também precisamos remover a anotação pendente que pode ter sido criada para ela.
