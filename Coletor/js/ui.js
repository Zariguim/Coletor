...
const UI = {
    ...
    init() {
        ...
        this.dom.openRelatorioBtn.addEventListener('click', () => {
             ...
             if(selectedFazenda){
                 RelatorioModal.open(selectedFazenda);
             } else {
                 alert("Selecione uma fazenda para gerar um relatório.");
             }
        });
        ...
        this.dom.historyFilter.addEventListener('change', () => {
            const talhao = MapManager.selectedLayer ? MapManager.selectedLayer.talhaoData : null;
            const fazenda = this.dom.fazendaSelect.value;
            if (fazenda) {
                 this.displayHistory(talhao?.nome_talhao, fazenda, talhao?.id);
            }
        });
    ...
    hideTalhaoInfo() {
        ...
        this.dom.monitoringHistory.innerHTML = ''; // Limpa o histórico ao des-selecionar
    },
    ...
    async displayHistory(talhaoName, fazendaName, talhaoId) {
        ...
        try {
            const [syncedHistory, localAnotacoes, pendingCsvs] = await Promise.all([
                getHistory(),
                talhaoId ? getAnotacoes(talhaoId) : Promise.resolve([]), // Só busca anotações locais se um talhão for especificado
                getPendingCsvs()
            ]);

            // Lógica de Filtragem Corrigida
            const filterLogic = r => {
                if (talhaoName) { // Visão de Talhão
                    // Mostra registros do talhão específico OU relatórios da fazenda
                    return r.fazenda === fazendaName && (r.talhao === talhaoName || (r.tag === 'Relatório' && r.talhao === fazendaName));
                } else { // Visão de Fazenda
                    // Mostra todos os registros da fazenda
                    return r.fazenda === fazendaName;
                }
            };
            
            const relevantSyncedHistory = syncedHistory.filter(filterLogic);
            const relevantPendingCsvs = pendingCsvs.filter(filterLogic).map(p => ({ ...p, isPending: true }));
            ...
            // Só adiciona anotações locais se estiver na visão de um talhão específico
            const localAnnotationRecords = talhaoName ? (localAnotacoes || []).map(a => ({ ...a, type: 'annotation', isLocal: true })) : [];
    ...
    async handleDeleteRecord(recordId, talhaoId, recordType, buttonElement, isPending, isLocal) {
        ...
        if (confirm(confirmationMessage)) {
            ...
            // CORREÇÃO: Lógica de exclusão de anotações
            if (recordType === 'annotation') {
                if (isLocal) {
                    // Se for local, apaga da lista de anotações e da fila de pendentes
                    await deleteAnotacao(talhaoId, recordId);
                    await deletePendingAnotacaoByLocalId(recordId);
                } else {
                    // Se já foi sincronizada, manda para a fila de exclusão do DataManager
