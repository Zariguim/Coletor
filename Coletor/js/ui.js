...
const UI = {
    ...
    async displayHistory(talhaoName, fazendaName, talhaoId) {
        ...
        try {
            ...
            const localAnnotationRecords = (localAnotacoes || []).map(a => ({ ...a, type: 'annotation', isLocal: true })); // Adiciona isLocal
    ...
    createDemarcationCard(record) {
        ...
        if (lines.length > 1) {
            // "description","kml_content","area_ha"
            const dataLine = lines[1];
            // Encontra a primeira e a segunda ocorrência de '","' para separar os 3 campos
    ...
    createAnnotationCard(record) {
        ...
        // CORREÇÃO: Passa isLocal para handleDeleteRecord
    ...
    async handleDeleteRecord(recordId, talhaoId, recordType, buttonElement, isPending, isLocal) {
        ...
        if (confirm(confirmationMessage)) {
            ...
            if (recordType === 'annotation') {
                if (isLocal) {
                    // Se for local, deleta diretamente do banco local
                    await deleteAnotacao(talhaoId, recordId);
                } else {
                    // Se não for local (ou seja, já sincronizada ou pendente), usa o fluxo normal
                    // A função deletePendingAnotacaoByLocalId será chamada se necessário
