...
const UI = {
    ...
    init() {
        ...
        this.dom.openRelatorioBtn.addEventListener('click', () => {
             if (MapManager.selectedLayer && MapManager.selectedLayer.talhaoData) {
                // Pega o nome da fazenda a partir do talhão atualmente selecionado
                const fazendaName = MapManager.selectedLayer.talhaoData.Fazenda || 'Sem Fazenda';
                RelatorioModal.open(fazendaName);
            } else {
                // Fallback caso o botão seja clicado sem um talhão selecionado
                const selectedFazenda = this.dom.fazendaSelect.value;
                if(selectedFazenda){
                    RelatorioModal.open(selectedFazenda);
                } else {
                    alert("Selecione uma fazenda ou um talhão para gerar um relatório.");
                }
            }
        });
    ...
    async displayHistory(talhaoName, fazendaName, talhaoId) {
        ...
        try {
            ...
            const relevantSyncedHistory = syncedHistory.filter(r =>
                r.fazenda === fazendaName &&
                (r.talhao === talhaoName || (r.tag === 'Relatório' && r.talhao === fazendaName))
            );
            const relevantPendingCsvs = pendingCsvs.filter(r =>
                r.fazenda === fazendaName &&
                (r.talhao === talhaoName || (r.tag === 'Relatório' && r.talhao === fazendaName))
