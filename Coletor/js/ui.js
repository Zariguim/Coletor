...
const UI = {
    ...
    showTalhaoInfo(talhao, areaEmHectares) {
        ...
        this.displayMonitoringHistory(talhao.nome_talhao, talhao.Fazenda || 'Sem Fazenda');
    },
    ...
    async displayMonitoringHistory(talhaoName, fazendaName) {
        this.dom.monitoringHistory.innerHTML = '<p class="text-center text-gray-500">Carregando histórico...</p>';
        try {
            const [localPendingCsvs, localHistoryCsvs] = await Promise.all([getPendingCsvs(), getHistory()]);
            ...
            [...relevantPending, ...relevantHistory].forEach(record => {
            ...
            if (allRecords.length === 0) {
                this.dom.monitoringHistory.innerHTML = '<p class="text-center text-gray-500">Nenhum monitoramento finalizado para este talhão.</p>';
                return;
            }
            ...
            allRecords.forEach(record => {
                // ... (lógica de cálculo de médias permanece a mesma) ...
                ...
                // NOVO: Adiciona a estrutura para o swipe
                historyHtml += `
            ...
            this.initSwipeToDelete(); // Inicia os listeners para o swipe
    ...
    initSwipeToDelete() {
        ...
        const handleDragStart = (e) => {
            // Se outro card estiver aberto, fecha ele primeiro
            if (this.activeSwipeCard && this.activeSwipeCard !== e.currentTarget.parentElement) {
                this.closeActiveSwipeCard();
            }
            isDragging = true;
            startX = e.pageX || e.touches[0].pageX;
            e.currentTarget.style.transition = 'none'; // Remove a transição durante o arrasto
        };
        ...
        const handleDragMove = (e) => {
            ...
            // Permite arrastar apenas para a esquerda, até um limite
            if (deltaX < 0) {
        ...
        const handleDragEnd = (e) => {
            ...
            cardContent.style.transition = 'transform 0.3s ease'; // Adiciona a transição de volta
            const deltaX = currentX - startX;

            // Se arrastou mais da metade do caminho, abre. Senão, fecha.
            if (deltaX < -40) {
                cardContent.style.transform = 'translateX(-80px)';
                this.activeSwipeCard = cardContent.parentElement; // Guarda o wrapper do card ativo
            } else {
