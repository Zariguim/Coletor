...
const RecomendacaoModal = {
        ...
        let html = `
            <div class="w-full max-w-2xl mx-auto">
                <div class="relative flex justify-between mb-2">
                    ...
                    <div class="absolute top-4 left-0 h-0.5 bg-blue-600 transition-all duration-300" style="width: ${progressWidth};"></div>
        `;
        steps.forEach((label, index) => {
            ...
            html += `
                <div class="z-10 flex flex-col items-center">
                    <div class="w-8 h-8 rounded-full flex items-center justify-center font-bold transition-all duration-300 ${isActive ? 'bg-blue-600 text-white border-2 border-blue-600' : 'bg-white text-gray-400 border-2 border-gray-300'}">
                        ${stepNumber}
                    </div>
                    <span class="text-xs mt-2 text-center ${isActive ? 'text-blue-700 font-semibold' : 'text-gray-500'}">${label}</span>
                </div>`;
        });
        html += `</div></div>`;
        ...
        let talhoesHtml = this.state.allTalhoes.map(talhao => `
            <div key="${talhao.id}" class="bg-white p-3 rounded-lg shadow-sm flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    <input type="checkbox" data-id="${talhao.id}" class="h-5 w-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500 talhao-checkbox" ${talhao.selecionado ? 'checked' : ''}>
                    <label class="font-medium text-gray-800">${talhao.nome}</label>
                </div>
                <div class="flex items-center space-x-2">
                    <input type="number" data-id="${talhao.id}" value="${talhao.area}" class="w-24 p-1 border border-gray-300 rounded-md text-right focus:ring-blue-500 focus:border-blue-500 talhao-area">
        ...
        this.dom.stepContent.innerHTML = `
            <div class="p-4 md:p-6">
                <div class="mb-6 bg-white p-4 rounded-lg shadow">
                    ...
                    <input type="number" id="remonte-input" value="${this.state.remonte}" class="w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500">
                </div>
                ...
                <div class="mt-8 sticky bottom-0 bg-white/80 backdrop-blur-sm -m-6 p-4 rounded-t-lg shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <span class="text-xl font-bold text-blue-600">${areaTotalSelecionada.toFixed(2)} ha</span>
                    </div>
                    <button id="talhoes-next-btn" class="w-full bg-blue-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-blue-700">Próximo</button>
                </div>
            </div>`;
        ...
        this.dom.stepContent.innerHTML = `
            <div class="p-4 md:p-6">
                <form id="add-product-form" class="bg-white p-4 rounded-lg shadow mb-6 flex gap-2">
                    ...
                    <button type="submit" class="bg-blue-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-blue-700">Adicionar</button>
                </form>
                ...
                <div class="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-sm p-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
                    <div class="max-w-2xl mx-auto flex gap-4">
                        ...
                        <button id="produtos-next-btn" class="w-full bg-blue-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-blue-700">Próximo</button>
                    </div>
                </div>
            </div>`;
        ...
        this.dom.stepContent.innerHTML = `
            <div class="p-4 md:p-6 space-y-6">
                ...
                <div class="bg-blue-50 border-l-4 border-blue-400 p-4 rounded-r-lg">
                    <p class="text-sm text-blue-700">Cada tanque fará <span class="font-bold">${hectaresPorTanque.toFixed(2)}</span> hectares.</p>
                </div>` : ''}
                 <div class="bg-white p-4 rounded-lg shadow">
                     <label class="block text-sm font-medium text-gray-700 mb-1">Observações</label>
                     <textarea id="obs-input" rows="4" class="w-full p-2 border border-gray-300 rounded-md" placeholder="Ex: Condições do tempo, cuidados especiais, etc.">${this.state.obs}</textarea>
                 </div>
                 <div class="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-sm p-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
                     <div class="max-w-2xl mx-auto flex gap-4">
                       <button id="aplicacao-back-btn" class="w-full bg-gray-200 text-gray-800 font-bold py-3 px-4 rounded-lg">Voltar</button>
                       <button id="aplicacao-next-btn" class="w-full bg-blue-600 text-white font-bold py-3 px-4 rounded-lg">Revisar Recomendação</button>
                    </div>
                </div>
             </div>`;
    ...
    _renderRevisaoScreen() {
        ...
        let html = `
            <div class="p-4 md:p-6 pb-24 space-y-6">
                <div class="bg-white p-4 rounded-lg shadow">
                    <h3 class="text-xl font-bold text-blue-700 mb-3 border-b pb-2">Resumo da Área</h3>
                    <div class="space-y-3 text-gray-700">
                        ...
                        <p class="pt-2 text-center text-lg">Serão necessários <span class="font-bold text-blue-600">${tanquesCheios}</span> tanques cheios e <span class="font-bold text-blue-600">${proporcaoTanqueParcial > 0.01 ? 1 : 0}</span> tanque parcial com <span class="font-bold text-blue-600">${(proporcaoTanqueParcial * 100).toFixed(0)}%</span> do volume.</p>
                    </div>
                </div>`;
        ...
        if (this.state.produtos.length > 0) {
            html += `
                <div class="bg-white p-4 rounded-lg shadow">
                    <h3 class="text-xl font-bold text-blue-700 mb-3 border-b pb-2">Produtos e Doses por Hectare</h3>
        ...
        if (tanquesCheios > 0) {
            html += `
                <div class="bg-white p-4 rounded-lg shadow">
                    <h3 class="text-xl font-bold text-blue-700 mb-3 border-b pb-2">Dosagem por Tanque Cheio</h3>
        ...
        if (proporcaoTanqueParcial > 0.01) {
            html += `
                 <div class="bg-white p-4 rounded-lg shadow">
                    <h3 class="text-xl font-bold text-blue-700 mb-3 border-b pb-2">Dosagem para Tanque Parcial</h3>
        ...
        if (this.state.obs) {
            html += `
                <div class="bg-white p-4 rounded-lg shadow">
                   <h3 class="text-xl font-bold text-blue-700 mb-2">Observações</h3>
        ...
        html += `
             <div class="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-sm p-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
                 <div class="max-w-2xl mx-auto flex gap-4">
                   ...
                   <button id="revisao-save-btn" class="w-full bg-blue-600 text-white font-bold py-3 px-4 rounded-lg">Salvar Recomendação</button>
                </div>
            </div>
        </div>`;
    ...
    async save() {
        ...
        const produtosString = this.state.produtos.map(p => `${p.nome} (${p.dose.toFixed(2)} ${p.unidade})`).join('; ');
