...
const DataManager = {
    ...
    async loadData() {
        ...
        // 1. Sempre tenta carregar os dados locais primeiro para uma inicialização rápida.
        try {
        ...
        if (navigator.onLine) {
            try {
                ...
                // Se a busca for bem-sucedida, atualiza o banco de dados e a UI.
                ...
                UI.populateFazendaSelect(fazendas);
                hasLocalData = true; // Confirma que temos dados.
            ...
            // Se estiver offline, informa o usuário.
            if (hasLocalData) {
                UI.setSyncStatus('Você está offline. Usando dados salvos.', 'warning', false);
            }
        }
    
        // 3. Verificação final para remover a tela de carregamento ou mostrar um erro.
        if (hasLocalData) {
            UI.setStatus(false); // Temos dados, então o aplicativo está pronto.
        } else {
            // Sem dados locais e offline.
            UI.setStatus(true, 'Nenhum dado encontrado. Conecte-se à internet para carregar.', false);
        }
        
        // 4. Sempre tenta sincronizar itens pendentes (se estiver online).
        this.syncAll();
    },
    ...
    /**
     * Converte um array de pontos de lat/lng para uma string KML.
     */
...
<coordinates>${coordinates}</coordinates>
    ...
    /**
     * Salva uma nova demarcação na fila de sincronização.
     */
