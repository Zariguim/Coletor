...
const DataManager = {
    ...
    /**
     * Carrega os dados iniciais, priorizando o cache local e sincronizando com o servidor.
     */
    async loadData() {
    ...
    /**
     * Finaliza todas as monitorias pendentes, gerando os CSVs e colocando na fila de sincronização.
     */
    ...
    /**
     * Sincroniza os CSVs pendentes com o servidor.
     */
    async syncPendingCsvs() {
        ...
        if (!navigator.onLine) {
            if (pendingCsvs.length > 0) {
                UI.setSyncStatus(`${pendingCsvs.length} CSV(s) aguardando para sincronizar.`, 'warning', false);
            }
            return;
        }
        ...
        if (successCount === totalCount) {
            ...
            UI.setSyncStatus(`Falha ao enviar ${remainingCount} CSV(s). Nova tentativa será feita quando houver conexão.`, 'warning', false);
        }
