...
const DataManager = {
    ...
    async loadData() {
        ...
        if (!currentUser) {
            UI.setStatus(true, 'Erro de autenticação. Por favor, faça login novamente.', false);
            return;
        }

        const userFazendas = currentUser.fazendas || [];
        ...
        try {
            ...
            if (localTalhoes && localTalhoes.length > 0) {
                // Filtra os talhões com base nas fazendas do usuário
                const filteredTalhoes = localTalhoes.filter(t => userFazendas.includes(t.Fazenda));

                MapManager.kmlDatabase = filteredTalhoes;
                const fazendas = [...new Set(filteredTalhoes.map(item => item.Fazenda || 'Sem Fazenda'))];
        ...
        if (navigator.onLine) {
            try {
                ...
                // Filtra os talhões no lado do servidor
                const { data: remoteData, error } = await supabaseClient
                    .from('talhoes')
                    .select('id, nome_talhao, conteudo_kml, Fazenda')
                    .in('Fazenda', userFazendas); // FILTRO APLICADO AQUI
        
                if (error) throw new Error(error.message);
        
                // Salva todos os talhões para uso offline por outros usuários, se necessário
                // mas a aplicação só mostrará os filtrados.
                const { data: allRemoteData } = await supabaseClient.from('talhoes').select('*');
                if (allRemoteData) await saveTalhoes(allRemoteData);
        ...
        if (hasLocalData) {
            ...
            UI.setStatus(true, 'Nenhum dado encontrado para seu usuário. Conecte-se à internet para carregar.', false);
        }
