...
const DataManager = {
    ...
    async loadData() {
        ...
        if (!hasLocalData && MapManager.kmlDatabase.length === 0) {
...
<coordinates>${coordinates}</coordinates>
     ...
     */
    async saveDemarcation(talhao, points, description) {
        // Adiciona o primeiro ponto ao final para fechar o polígono no KML
        const closedPoints = [...points, points[0]];
        const kmlContent = this.pointsToKML(closedPoints, description);

        // O conteúdo KML é grande, então o colocamos em uma "célula" do CSV.
        const csvData = `description,kml_content\n"${description.replace(/"/g, '""')}","${kmlContent.replace(/"/g, '""')}"`;
