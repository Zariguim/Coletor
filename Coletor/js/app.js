...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const dom = {
        ...
        // NOVO: Elementos da barra de progresso
    ...
    let isSyncing = false; // NOVO: Flag para evitar sincronizações simultâneas
    ...
    // FUNÇÃO ATUALIZADA para controlar a barra de progresso
    function setSyncStatus(text, type = null, autoHide = true, showProgressBar = false) {
        ...
        if (text && autoHide) {
             setTimeout(() => setSyncStatus('', null), 4000); // Aumentado o tempo para 4s
        }
    }
    ...
    function showTalhaoInfo(talhao, clickedLayer) {
        ...
        const key = `registros_${talhao.id}`;
    ...
    async function downloadTalhaoCSV() {
        ...
        if (!dataJSON) {
            alert('Nenhum dado de alvo foi salvo para este talhão.');
            return;
        }

        const records = JSON.parse(dataJSON);
        if (!records || records.length === 0) {
            alert('Não há registros para gerar o CSV.');
            return;
        }

        const allAlvos = new Set();
        records.forEach(record => {
            record.alvos.forEach(alvo => allAlvos.add(alvo));
        });
        ...
        const filename = `${talhao.nome_talhao.replace(/ /g, '_')}_${dateString}.csv`;
        ...
        try {
            ...
            setSyncStatus('CSV salvo na fila para sincronização.', 'syncing');
            syncPendingCsvs();
        } catch (error) {
            console.error('Erro ao salvar CSV na fila de pendentes:', error);
            setSyncStatus('Erro ao enfileirar CSV.', 'warning');
        }
    }

    // FUNÇÃO TOTALMENTE REESCRITA para melhor feedback e robustez
    async function syncPendingCsvs() {
        if (isSyncing) return; // Evita execuções múltiplas
        ...
        for (const record of pendingCsvs) {
            try {
                ...
                // Se o upload foi bem-sucedido, remove da fila local
                ...
                // O item permanecerá na fila para a próxima tentativa
            }
            
            // Atualiza a barra de progresso após cada tentativa (sucesso ou falha)
    ...
    dom.downloadCsvBtn.addEventListener('click', downloadTalhaoCSV);
    ...
    // NOVO: Verifica a cada 30 segundos se há algo para sincronizar
