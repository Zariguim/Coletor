...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let monitoringPinsGroup = null; // Grupo para os pins de monitoria
    let kmlDatabase = [];
    ...
    function initMap() {
        ...
        monitoringPinsGroup = L.featureGroup().addTo(map); // Adiciona o grupo de pins ao mapa
    }
    ...
    function drawMonitoringPins() {
        monitoringPinsGroup.clearLayers();
                ...
                if (records && records.length > 0) {
                    records.forEach(record => {
                        if (record.lat && record.lng) {
                            ...
                            monitoringPinsGroup.addLayer(marker);
                        }
                    });
                }
            }
    ...
    async function finalizeAllMonitoring() {
        ...
        syncPendingCsvs();
        monitoringPinsGroup.clearLayers(); // Limpa os pins do mapa
    }
    ...
    initMap();
    loadData();
    registerServiceWorker(); // CORRIGIDO
    updateFinalizeButtonVisibility();
    drawMonitoringPins(); 
