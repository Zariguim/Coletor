...
const RelatorioModal = {
    ...
    async addRecordBox(pdf, record, y, checkPageBreak) {
        ...
        const textHeight = textLines.length * (10 * 0.45); // CORREÇÃO: Fator de altura de linha aumentado
        ...
        y = checkPageBreak(y, totalBoxHeight + 5); // CORREÇÃO: Passa 'y' como argumento
    ...
    async generatePdf() {
        ...
        try {
            ...
            // CORREÇÃO: Função pura para checar quebra de página, sem depender de 'y' externo
            ...
            // CORREÇÃO: Helper que usa a nova função checkPageBreak e gerencia a posição 'y'
            const addText = (text, size, style) => {
                const maxWidth = pdf.internal.pageSize.width - margin * 2;
                const lines = pdf.splitTextToSize(text, maxWidth);
                const textHeight = lines.length * (size * 0.45); // Fator de altura aumentado
                y = checkPageBreak(y, textHeight + 2);
                pdf.setFontSize(size).setFont('helvetica', style).text(lines, margin, y);
                y += textHeight + 2;
            };

            // CORREÇÃO: Helper para linha
            const addLine = () => {
            ...
            for (const talhaoName in selectedRecordsByTalhao) {
                // CORREÇÃO: Lógica de quebra de página entre talhões
                const talhaoHeaderHeight = 40; // Altura estimada para o cabeçalho do talhão
                if (!isFirstTalhao) {
                    y = checkPageBreak(y, talhaoHeaderHeight);
                }
                isFirstTalhao = false;
                ...
                for (const tag of tagOrder) {
                    if (groupedRecords[tag]) {
                        ...
                        for (const record of groupedRecords[tag]) {
                            // CORREÇÃO: Passa a função checkPageBreak pura para o método
    ...
    async uploadAndSaveReport(pdfBlob) {
        ...
        const sanitizedFazendaName = this.state.currentFazenda
            .normalize("NFD")
            .replace(/[\u0300-\u036f]/g, "")
            .replace(/\s+/g, '_')
            .replace(/[^a-zA-Z0-9_.-]/g, '');

        const fileName = `relatorio_${sanitizedFazendaName}_${Date.now()}.pdf`;
        ...
        try {
            ...
            const recordToSync = {
                ...
                talhao: this.state.currentFazenda,
